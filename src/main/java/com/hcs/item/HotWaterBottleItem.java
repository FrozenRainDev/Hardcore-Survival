...
//                nbt.putFloat(HHEP, -1.0F);//Just chilled
    ...
    public static float getUnsignedPercentByInitTimeAdvanced(World world, @NotNull ItemStack stack) {
        ...
        NbtCompound nbt = stack.getOrCreateNbt();
        return nbt.contains(HHS) && nbt.getInt(HHS) > 0 ? 1.0F - getUnsignedPercentByInitTime(world, nbt.getLong(HHCI)) : getUnsignedPercentByInitTime(RotHelper.theWorld, nbt.getLong(HHCI));
    }
...
//        tooltip.add(Text.of(nbt.toString()));
    ...
    public int getItemBarStep(ItemStack stack) {
        if (stack == null || RotHelper.theWorld == null) return super.getItemBarStep(stack);
        NbtCompound nbt = stack.getOrCreateNbt();
        return Math.round(13.0F * (nbt.contains(HHCI) ? getUnsignedPercentByInitTimeAdvanced(RotHelper.theWorld, stack) : getExpPercent(RotHelper.theWorld, stack, stack.getOrCreateNbt().contains(HHES))));
    }
