...
public class ColdWaterBottleRecipe extends CustomRecipe {

    public ColdWaterBottleRecipe(ResourceLocation id, CraftingBookCategory category) {
        super(id, category);
    }
    ...
    public boolean matches(@NotNull CraftingContainer inventory, Level world) {
        ...
        for (int i = 0; i < inventory.getContainerSize(); ++i) {
            ...
            if (!stack.isEmpty()) {
                ++count;
                //Watter bottle
                if (stack.getItem() == Items.POTION && stack.getOrCreateTag().equals(Items.POTION.getDefaultInstance().getOrCreateTag()))
                    b1 = true;
