    ...
    public static void init() {
        ClientPlayConnectionEvents.JOIN.register(((handler, sender, client) -> {
            ...
            client.player.sendMessage(Text.translatable(combineText("itemGroup.hcs.main", " Beta %1$s "), UpdateHelper.MOD_VER)
                            ...
                            .append(Text.literal(" ")).append(Text.translatable("hcs.tip.config").formatted(Formatting.UNDERLINE, Formatting.AQUA).styled(style -> style.withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, OFFICIAL_URL + "tutorial.html"))))
                    , false);
