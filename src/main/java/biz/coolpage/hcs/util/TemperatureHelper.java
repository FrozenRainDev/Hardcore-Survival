...
public abstract class TemperatureHelper implements LevelReader {
    ...
    public static float getTemp(Level world, BlockPos pos, boolean allowRecursion) {
        try {
            float temp = 0.5F;
            if (world == null || pos == null) {
                Hcs.error("TemperatureHelper/getTemp;world or pos is null");
                return temp;
            }
            ...
            if (world.dimension() == Level.OVERWORLD) {
                ...
                if ((biomeName.contains("river") || biomeName.contains("beach")) && allowRecursion) {
                    ...
                    for (int[] bp : POS_IN_NEARBY_CHUNKS) {
                        list.add(getTemp(world, pos.offset(bp[0], 0, bp[1]), false));
                    }
                    float tempSum = 0.0F;
                ...
                return getDailyTemp(world.getDayTime(), temp, dailyTempAmplitude);
            } else return temp;
        } catch (StackOverflowError error) {
            Hcs.error("TemperatureHelper/getTemp(): StackOverflowError");
            return 0.5F;
        }
    }
    ...
    public static float getTemp(Object playerObj) {
        if (playerObj instanceof ServerPlayer player)
            return updateEnvTempCache(player, getTemp(player.level(), player.blockPosition(), true));
        else if (playerObj instanceof Player player) //ClientPlayer
    ...
    public static void updateAmbientBlocks(Entity playerObj) {
        if (playerObj instanceof ServerPlayer player) {
            int i = (int) player.level().getGameTime() % (BALL_RAD3.length);
            ...
            int[][] bp = BALL_RAD3[i];
            for (int[] bpp : bp) {
                BlockPos checkPos = playerPos.offset(bpp[0], bpp[1], bpp[2]);
                net.minecraft.world.level.block.state.BlockState state = player.level().getBlockState(checkPos);
                Block block = state.getBlock();
                Holder<Biome> biomeEntry = player.level().getBiome(playerPos);
                Biome biome = biomeEntry.value();
                if (biome != null) {
                    if (!biome.coldEnoughToSnow(playerPos) && !getBiomeName(biomeEntry).contains("frozen")) {
                        ...
                        else if (block == Blocks.SNOW || block == Blocks.SNOW_BLOCK || block == Blocks.POWDER_SNOW || block instanceof PowderSnowCauldronBlock)
                ...
                else if ((block instanceof AbstractFurnaceBlock && state.getValue(AbstractFurnaceBlock.LIT)) || block == Blocks.FIRE)
                    temperatureManager.addAmbient(0.3F);
                else if (block == Blocks.MAGMA_BLOCK)
                    temperatureManager.addAmbient(0.5F);
                else if (block == Blocks.LAVA || block instanceof LayeredCauldronBlock && state.getValue(LayeredCauldronBlock.LEVEL) > 0) // Approximation for lava cauldron
                    temperatureManager.addAmbient(10.0F);
                //Addition: check oxygen generation
                if ((block instanceof LeavesBlock || (block instanceof BushBlock && !(block instanceof RootsBlock) && block != Blocks.DEAD_BUSH) || block == Blocks.GRASS_BLOCK)) {
                    for (BlockPos immediatePos : new BlockPos[]{checkPos.above(), checkPos.below(), checkPos.east(), checkPos.south(), checkPos.west(), checkPos.north()}) {
                        if (player.level().getBrightness(LightLayer.BLOCK, immediatePos) > 5)
    ...
    public static float updateEnvTempCache(ServerPlayer player, float val) {
        TemperatureManager temperatureManager = ((StatAccessor) player).getTemperatureManager();
    ...
    public static float getFeelingTemp(Object playerObj, float envTemp, String biomeName, int skyLightLevel) {
        ...
        } else if (playerObj instanceof Player player) {
            //ClientPlayer
    ...
    public static int getWindchillLevel(Level world, BlockPos pos, Holder<Biome> biomeEntry) {
        int result = 0;
        if (world == null || pos == null || biomeEntry == null) {
            Hcs.error("TemperatureHelper/getWindchillLevel; world, pos, biomeEntry is null");
            return result;
        }
        Biome biome = biomeEntry.value();
        if (!biome.coldEnoughToSnow(pos) || world.getBrightness(LightLayer.SKY, pos) < 1 || (!world.canSeeSky(pos) && world.getBrightness(LightLayer.BLOCK, pos) > 7))
